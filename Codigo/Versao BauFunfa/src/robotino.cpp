...
Robotino::Robotino(const char *hostname,
    ...
    try{
        ...
        coresFaltando = std::vector<int>();
                                            //SKIRA ADICIONA AREAS
                                                this->criaAreas();
                                            //TERMINA ADICIONA AREAS
    }
...
void Robotino::removerCorFaltando(int cor){
 ...
 void Robotino::criaAreas()
 {
    //CODIGO DAS AREAS
        ...
        for(int i = 0; i < 6; i++)
        {
            this->a.push_back(Area(i));
        }
    //DEFINE AREAS END

 }

 int Robotino::getArea()//GET REEEEEEECT
    {
        Coordenadas teste;
        bool condArea;
        bool naoEcziste = true;
        Coordenadas robotinoAtual;

        robotinoAtual = Coordenadas(this->odometryX(),this->odometryY());

        for(int backstreetsback = 0; backstreetsback < 6; backstreetsback++)
        {
            teste = this->a[backstreetsback].pegar_coordenada(this->a[backstreetsback].carea);

            std::cout << "PIROCA \t" << backstreetsback <<"\n\n\n\n"<<std::endl;
            condArea = (abs(robotinoAtual.get_x() - teste.get_x()) < XYAErro)&&(abs(robotinoAtual.get_y() - teste.get_y()) < XYAErro);
            std::cout<<"RobotinoAtualX =\t"<<robotinoAtual.get_x()<< "\tRobotinoAtualY =\t"<< robotinoAtual.get_y() <<std::endl;
            // std::cout<< <<std::endl;
            if (condArea)
            {
                std::cout << "\n\n\n\nDENTRO DA AREA"<< backstreetsback <<"\n\n\n\n"<<std::endl;
                naoEcziste = false;
                return backstreetsback;
            }
        }
        if(naoEcziste)
        {
