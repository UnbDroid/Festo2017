...
IrDistPelaParede  *IrDistPelaParede::instance()
{
    static IrDistPelaParede  IrParedePelaParedeStateInstance;
    return &IrParedePelaParedeStateInstance;
}

IrDistPelaParede::IrDistPelaParede() {}

IrDistPelaParede::~IrDistPelaParede() {}

void IrDistPelaParede::enter(Robotino *robotino)
{
    std::cout << "Entrando no estado IrDistPelaParede...\n";
}

void IrDistPelaParede::execute(Robotino *robotino)
{
    ...
    float direcao = 0;
    float erroDist = 0;
    float erroDistEsq = 0, erroDistDir = 0;
    ...
    static float cos20 = std::cos(20*PI/180), cos10 = std::cos(10*PI/180), cos30 = std::cos(30*PI/180);
    ...
    if(paredeAlvo == Robotino::NORTEN90  ||  paredeAlvo == Robotino::SUL90 || paredeAlvo == Robotino::NORTE90  ||  paredeAlvo == Robotino::SULN90){
        if(paredeAlvo == Robotino::NORTEN90  ||  paredeAlvo == Robotino::SULN90){
            distRef = -robotino->yD+robotino->odometryY();
        }
        else{
            distRef = robotino->yD-robotino->odometryY();
        }
        ...
        if(paredeAlvo == Robotino::OESTE0 || paredeAlvo == Robotino::LESTE0){
            distRef = robotino->xD-robotino->odometryX();
        }
        else {
            distRef = -robotino->xD+robotino->odometryX();
        }
    ...
    if ((paredeAlvo == Robotino::NORTEN90  || paredeAlvo == Robotino::OESTE0 || paredeAlvo == Robotino::SUL90 || paredeAlvo == Robotino::LESTE180) && girei && !ajuste ) {
        std::cout<<"XABLAU EU TERMINEI DE GIRAR E AGORA VAMO VER QUE MERDA EU TO FAZENDO\n\n\n";
        ...
        distancia_da_esquerda= ((e1+R)*cos10 + (e2+R)*cos30)/2;

        std::cout<<"distancia_da_esquerda: "<<distancia_da_esquerda<<"\n\n";

        erroDistEsq = refDist - (distancia_da_esquerda -20);

        Vy = -Kpy*erroDistEsq;

        std::cout<<"Vy: "<<Vy<<"\n\n";

        erro = ((((refDist+R)/cos10) - (e1+R)) - (((refDist+R)/cos30) - (e2+R)))/2;
        w = -Kp*erro;
        std::cout<<"\n\nerro do w: "<<erro<<"\n\n";
        std::cout<<"\n\ne1: "<<e1<<"\n\n";
        std::cout<<"\n\ne2: "<<e2<<"\n\n";
        ...
        distancia_da_direita= ((d1+R)*cos10 + (d2+R)*cos30)/2;//((d1+d2+2*R)*cos20)/2;

        std::cout << "distancia_da_direita: " << distancia_da_direita <<"\n";
        std::cout << "distRef: " << distRef <<"\n";

        erroDistDir = refDist - (distancia_da_direita - 20);

        Vy = Kpy*erroDistDir;
        std::cout << "Valor de Vy: " << Vy << std::endl;

        erro = ((((refDist+R)/cos10) - (d1+R)) - (((refDist+R)/cos30) - (d2+R)))/2;//cos10*(d1+R) - cos30*(d2+R);
        w = Kp*erro;
    ...
    if ((paredeAlvo == Robotino::NORTEN90  || paredeAlvo == Robotino::OESTE0 || paredeAlvo == Robotino::SUL90 || paredeAlvo == Robotino::LESTE180) && ajuste ){
        ...
        erro = (((refDist/cos10) - d1) - ((refDist/cos30) - d2))/2;
        erro_int += erro*dt;
        w = Kp*erro+Ki*erro_int;
        robotino->setVelocity(0,0,w);
        ...
        if (std::abs(erro) < limiar2){
            ...
            erro_int = 0;
            robotino->setVelocity(0,0,0);
            distancia_da_direita = ((d1+R)*cos10 + (d2+R)*cos30)/2;
...
void IrDistPelaParede::exit(Robotino *robotino) {}