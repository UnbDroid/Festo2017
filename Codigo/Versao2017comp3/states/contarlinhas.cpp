...
void ContarLinhas::execute(Robotino *robotino)
{
    ...
    if (preto || tag) {
	 std::cout << "ACHEIeieie!"<<numeroLinhas << std::endl;
        tag = true;
        if (!preto){
            numeroLinhas++;
            tag = false;
	    std::cout << "ACHEIeTROLOLO!  \n\n\n\n\n\n\n\n\n\n\n\n\n\n "<<numeroLinhas << std::endl;
        }
    }
    ...
    if((robotino->readInductiveSensor() > 9.9 && !viMetal) || !robotino->readInductiveSensor()){
        ...
        if (preto){
            numeroLinhas ++;
		std::cout << "ACHEI! "<<numeroLinhas << std::endl;
        }
 	std::cout << "Numero Linhas: "<<numeroLinhas << std::endl;
        ...
        if (numeroLinhas == 1){
            robotino->setNumeroLinhas(numeroLinhas);
	//	robotino->setOdometry(40 , robotino->odometryY(), 180);
