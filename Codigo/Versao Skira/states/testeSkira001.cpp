...
testeSkira001  *testeSkira001::instance()
{
    static testeSkira001  testeSkira001StateInstance;
    return &testeSkira001StateInstance;
}

testeSkira001::testeSkira001() {}

testeSkira001::~testeSkira001() {}

int identificarCorAreaTestar(Robotino *robotino){
...
void testeSkira001::enter(Robotino *robotino)
{
    std::cout << "Entrando no estado testeSkira001...\n";
}



int definirOndePegarTestar (NoBusca& melhorResultado, int numPassosFazer, int &numPassosFeitos){
...
int definirOndeDeixarTestar(NoBusca& melhorResultado,int area, int &numPassosFeitos,int numPassosFazer){
...
void testeSkira001::execute(Robotino *robotino)
{
...
// Veio direto pra cá -----------------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[5] = identificarCorAreaTestar(robotino);
 ...
 // Indo para a area de deposito 5 identificar a cor -----------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[4] = identificarCorAreaTestar(robotino);
...
// Indo para a area de deposito 4 identificar a cor -----------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[3] = identificarCorAreaTestar(robotino);
...
// Indo para a area de deposito 3 identificar a cor -----------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[2] = identificarCorAreaTestar(robotino);
    ...
    // Indo para a area de deposito 2 identificar a cor -----------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[1] = identificarCorAreaTestar(robotino);
 ...
 // Indo para a area de deposito 1 identificar a cor ------------------------------------------------------------------------------------------------------------------------------
        ...
        ordemAtualDiscos[0] = identificarCorAreaTestar(robotino);
...
// Planeja como organizar os discos ------------------------------------------------------------------------------------------------------------------------
        ...
        objetivo_completo = definirOndePegarTestar(melhorResultado, numPassosFazer, numPassosFeitos);
...
// Ir para area 1  ---------------------------------------------------------------------
 ...
 // Rotina pra ir coletar o disco -------------------------------------------------------------------------------------------------------------------------------
        ...
        objetivo_completo = 28;
         ...
         identificarCorAreaTestar(robotino);
    ...
    // Decide o destino do disco e difere entre deposito e area intermediaria ------------------------
        ...
        robotino->change_state(IrParaPonto::instance());
        objetivo_completo = definirOndeDeixarTestar(melhorResultado, robotino->getDepositoDestino(),numPassosFeitos, numPassosFazer);
        deixando = true;
...
// Voltar da area intermediaria ----------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if(!deixando){
            ...
            objetivo_completo = 391;
        }

    }else if (objetivo_completo == 391){
        ...
        robotino->definirDestino(robotino->odometryX()/10+RE+20, robotino->odometryY()/10);
        robotino->change_state(IrParaPonto::instance());
        if(!deixando){
             objetivo_completo = 19;
        }else{
 ...
 // Deixar Discos a partir do deposito ----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // Indo deixar o disco ------------------------------------------------------------------------------------
        ...
        robotino->setCarregando(false);
        if (robotino->getDepositoDestino() == Robotino::AREA7){
...
// Coletar disco na area intermediaria ----------------------------------------------------------------------------------------------------------------------------
        ...
        robotino->change_state(IrParedePelaParede::instance());
        objetivo_completo = 4721;

    }else if (objetivo_completo == 4721) {

        robotino->change_state(IdentificarCor::instance());
        objetivo_completo =472;
    }
    else if (objetivo_completo == 472) {


         robotino->setVelocidadeBusca(40);
        robotino->definirCorAlvo(Robotino::TODAS);
        robotino->change_state(ProcurarCor::instance());
         ...
         deixando = true;
         //robotino->setCarregando(true);
         objetivo_completo = 481;

    }else if (objetivo_completo == 481) {
        robotino->setThetaR(-90);
        robotino->change_state(Girar::instance());
        objetivo_completo = 37;
    }
...
// Voltar para o Inicio -----------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        robotino->definirDestino((robotino->odometryX())/10, robotino->odometryY()/10+38);
        robotino->change_state(IrParaPonto::instance());
...
void testeSkira001::exit(Robotino *robotino) {}
